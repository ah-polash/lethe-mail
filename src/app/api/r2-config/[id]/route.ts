...
export async function PUT(
  ...
  try {
    ...
    const existing = await prisma.r2Config.findUnique({ where: { id } });
    ...
    const config = await prisma.r2Config.update({
      where: { id },
      data: {
        ...(name !== undefined && { name }),
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.r2Config.delete({ where: { id } });
...
export async function PATCH(
  ...
  try {
    ...
    await prisma.r2Config.updateMany({ data: { isActive: false } });
    const config = await prisma.r2Config.update({
      where: { id },
      data: { isActive: true },
    });
